# 0.3: Xây dựng hệ thống quản lý bán hàng bằng lập trình hướng đối tượng

# - Thông tin về 1 sản phẩm bao gồm các thuộc tính như: 
# mã sản phẩm, tên sản phẩm, thương hiệu, loại sản phẩm, giá. 
# Loại sản phẩm có thể là: Thiết bị điện tử (điện thoại, laptop, máy tính bảng, ...), 
# đồ gia dụng (quạt, máy pha cà phê, ...), 
# mã sản phẩm là số nguyên không âm và không trùng nhau.

# - Thông tin về khách hàng gồm có (mã khách hàng, họ tên, số điện thoại). 
# Mã khách hàng là dạng string kiểu uuid

# Trong một đợt khuyến mại trong 3 ngày để tri ân khách hàng, cửa hàng cho phép 
# khách hàng được mua không quá 5 sản phẩm trong 1 ngày với giá ưu đãi là 50%. 
# Nếu mua quá 5 sản phẩm thì sản phẩm thứ 6 được tính với giá 100%

# Yêu cầu

# - Định nghĩa các lớp thoả mãn các yêu cầu trên
# - Tạo hàm nhập sản phẩm mới và lưu vào tệp products.txt, có xử lý ngoại lệ, ràng buộc chặt chẽ các điều kiện. Ví dụ: Giá sản phẩm > 0. mã khách hàng buộc phải dạng uuid. Sản phẩm cùng mã thì không cho thêm vào
# - Tạo hàm nhập khách hàng mới vào tệp customers.txt, khách hàng trùng id là không được phép., có xử lý ngoại lệ, ràng buộc chặt chẽ các điều kiện
# - Lập báo cáo mua hàng cho từng khách hàng (nếu có) 
# - Sắp xếp danh sách mặt hàng đã bán theo: tên khách hàng, thời gian mua hàng 
import uuid
global max_discount
max_discount = 5
...
def choice_after_buying():
    ...
    # order_id = uuid.uuid4
    # order_cus_id = input("Mã kh: ")
    # order_pro_id = input("Mã sp: ")
    # order_pro_quantity = input("Số lượng sp: ")
    # f1 = open('products.txt', 'r+', encoding='utf-8')
    # lines = f1.readlines()
    # for line in lines:
    ...
    #     c = line.split(',')
    #     try:
    #         str(order_pro_id) == c[0]
    #         print("id sp hợp lệ")
    #         break
    #         # try:
    #         #     if int(order_pro_quantity)
            
    #     except:
    #         print(f"id sp k hợp lệ lrr4.")
    #         cus_order()
